...
| 항목 | 레이블 (`labels.ts`) | 설명 | 상태 |
|---|---|---|---|
| 보호구 미착용 | `no_helmet`, `no_vest` | 안전모/안전조끼 미착용 인원 감지 | 추론 파이프라인 구현됨 (모델 가중치는 사용자 제공 필요) |
| 위험지역 진입 | `danger_zone_intrusion` | 추락/협착 위험 구역 무단 진입 | 〃 |
| 화재/불꽃 | `fire_smoke` | 카메라 스트림 내 화재 징후 (고위험, 즉시 알림) | 〃 |
| 쓰러짐(Man-Down) | `man_down` | 쓰러진 자세의 인체 감지 (고위험, 즉시 알림) | 〃 |
...
- 학습된 객체 감지 모델 가중치 (보호구/위험지역/화재/쓰러짐 4개 클래스, `.onnx` 형식) — `web/public/models/README.md` 참고
